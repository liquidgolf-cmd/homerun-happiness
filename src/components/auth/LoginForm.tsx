...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (error) {
      ...
      // Login successful - check for stored assessment (B2)
      if (data?.user?.id) {
        try {
  ...
  const handleGoogleLogin = async () => {
    ...
    // Note: OAuth will redirect away, so we don't navigate here
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 px-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-300 text-gray-700 py-3 px-4 rounded-loam font-medium hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition mb-6"
        >
        ...
        <div className="mt-6 text-center">
          <p className="text-gray-600">
            ...
            <Link to="/signup" className="text-loam-brown font-medium hover:underline">
              Sign up
            </Link>
          </p>
        </div>
