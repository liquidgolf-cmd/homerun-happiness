...
export default function FirstBase() {
  ...
  useEffect(() => {
    if (conversation) {
      const stageCompleted = isStageCompleted('first_base');
      const isPastStage = conversation.current_base !== 'first_base' && 
        ['second_base', 'third_base', 'home_plate', 'completed'].includes(conversation.current_base);
...
In that moment, how were you showing up? What were you doing differently than usual?`;
      ...
      const { data, error } = await messagesApi.updateMessage(firstAssistantMsg.id, { content: newMsg });
      if (error || !data) {
        console.warn('First Base migration: update failed (0 rows affected). Run the SQL migration in Supabase: database-migration-messages-update-policy.sql', error || new Error('No rows updated'));
      } else {
        reload();
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-loam-neutral to-loam-neutral px-4 py-8">
      <div className="max-w-6xl mx-auto">
        ...
        {isReviewMode && !allowContinue && (
          <div className="mb-6 bg-amber-50 border-2 border-amber-300 rounded-loam p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-amber-900">Reviewing First Base - This conversation is complete</h3>
        ...
        <ProgressBar currentBase="first_base" completedStages={completedStages} />
